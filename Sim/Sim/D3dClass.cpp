...
bool D3DClass::Initialise(int screenWidth, int screenHeight, bool vsync, HWND hwnd, bool fullscreen, float screenDepth, float screenNear)
{
	...
	error = wcstombs_s(&stringLength, m_videoCardDescripton, 128, adapterDesc.Description, 128);
	if (error != 0)
	{
		return false;
	}
...
void D3DClass::GetProjetionMatrix(XMMATRIX& projectionMatrix)
{
...
{
	strcpy_s(cardName, 128, m_videoCardDescripton);
