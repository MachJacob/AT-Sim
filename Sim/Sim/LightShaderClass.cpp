...
bool LightShaderClass::Initialise(ID3D11Device* device, HWND hwnd)
{
	...
	WCHAR vs[] = L"../Sim/light.vs";
	WCHAR ps[] = L"../Sim/light.ps";
	result = InitialiseShader(device, hwnd, vs, ps);
	if (!result)
	{
		return false;
	}
	...
	XMMATRIX projectionMatrix, ID3D11ShaderResourceView* texture, XMFLOAT3 lightDirection, XMFLOAT4 diffuseColour, XMFLOAT4 ambientColour)
{
	...
	result = SetShaderParameters(deviceContext, worldMatrix, viewMatrix, projectionMatrix, texture, lightDirection, diffuseColour, ambientColour);
	if (!result)
	{
		return false;
	}
...
void LightShaderClass::OutputShaderErrorMessage(ID3D10Blob* errorMessage, HWND hwnd, WCHAR* shaderFilename)
{
	...
	MessageBox(hwnd, L"Error compiling shader. Check shader-error.txt for messsage.", shaderFilename, MB_OK);
